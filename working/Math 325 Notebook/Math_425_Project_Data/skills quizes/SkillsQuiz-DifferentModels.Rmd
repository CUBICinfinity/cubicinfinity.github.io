...
curvy_lm <- lm(gasbill ~ month + I(month^2), data = Utilities)
v <- coef(curvy_lm)
curve(v[1] + v[2]*x + v[3] * x^2, add = T, col = "skyblue", lwd = 2)
```
...
```{r}
predict(curvy_lm, data.frame(month = 9), interval = "prediction")
```

the lower prediction is 0, not negative
...
```{r}
mean(subset(Utilities, month == 9)$gasbill)

38.84658 - 22.815

plot(gasbill ~ month, data=Utilities, main="Single Residence in Minnisota", xlab="Month of the Year", ylab="Monthly Gas Bill (US Dollars)")
curve(v[1] + v[2]*x + v[3] * x^2, add = T, col = "skyblue", lwd = 2)
abline(h = mean(subset(Utilities, month == 9)$gasbill), lty = 3)
```
...
```{r}
summary(curvy_lm)
sum(curvy_lm$res^2)/114 # becasue estimated 3 values
```
