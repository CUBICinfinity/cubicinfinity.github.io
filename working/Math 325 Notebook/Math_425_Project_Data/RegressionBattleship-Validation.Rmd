...
```{r}
set.seed(125) #This ensures the randomness is the "same" everytime if you play the entire R-chunk as one entire piece of code. If you run lines separately, your data might not come out the same every time.
...
## Then, create betas, errors (by choosing sigma), and Y
 ...
 Y <- beta0 + beta1*X2 + errors #...edit this code and replace it with your model
...
write.csv(RBdata, "RBdata2.csv", row.names=FALSE)
...
```{r}
d1 <- read.csv("RBdata.csv", header=TRUE)
d2 <- read.csv("RBdata2.csv", header=TRUE)
```


```{r}
lmTrue <- lm(Y ~ X2, data=d1)
lmBS <- lm(Y ~ X2 + I(X2^2), data=d1)
lms1 <- lm(Y ~ X3 + X2 + sqrt(X2), data=d1)
lms2 <- lm(Y ~ X2, data=d1)
```

| Model | R-squared |
|-------|--------------------|
| True | `r summary(lmTrue)$r.squared` |
| Brother Saunders | `r summary(lmBS)$r.squared` |
| Student 1 | `r summary(lms1)$r.squared` |
| Student 2 | `r summary(lms2)$r.squared` |
