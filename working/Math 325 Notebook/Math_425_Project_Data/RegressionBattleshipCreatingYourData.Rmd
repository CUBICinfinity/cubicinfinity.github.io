...
$$
  Y_i = \beta_0 + \beta_1 X_{4i} + \beta_2 X_{2i} + \beta_3 X_{4i} X_{2i} + \epsilon_i
$$
...
X1 <- runif(n,0,4) 
 ...
 X6 <- sample(c('John', 'Henry', 'Jessie', 'Sarah', 'Cory', 'Thor', 'Amelia', 'S C Kennedy', 'Big Max', 'Orion', 'Ferris', 'Kystystal', 'Zach', 'Jacqueline', 'Emily', 'Curious George', 'Alice', 'Francine', 'Johnny James Yogurt Jr', 'Bobo', 'Olivia', 'Marcel the Shell', '(─‿‿─)', 'Moose'),  size = n, replace = T)

 X7 <- sample(rnorm(n, 9, 1.3) - seq(1, 3, length.out = n), n)/6
 X8 <- 1/sample(rnorm(n, 0, 7) - seq(1, 3, length.out = n), n)/X7 + rnorm(n, -3, 1)
 X7 <- -(X7 + X8 + rnorm(n, -3, 1)^2)
 
 X9 <- rep(9, n)
 X9[29] <- 1
 
 X10 <- sample(seq(0.01, 10, length.out = n), n) + rnorm(n, 0, 0.2) + X8/3
 X9 <- ifelse(X10 + rnorm(n, 1, 1) > 3.9, 1, 9)
 
 X7 <- X7 + X9/9 + X8/10 
 
 X6 <- paste0(X6, " - ", round(X7^2))
 
X4 <- runif(n, -1000, 1000)^X10

X5 <- 8.2 + ifelse(X7 > 2, 0   + rnorm(n, 0, .33), ifelse(X7 < -1, 1 + rnorm(n, 0, .15), ifelse(runif(n, 0, 1) > 0.5, 2  + rnorm(n, 0, .33), 3 + rnorm(n, 0, .33))))
X5 <- X5 - 2*log(X8 + 1000) + X1

 a <- X8[45]
 X8[45] <- X9[45]
 X9[45] <- a

 View(cbind(X4, X6, X7, X8, X9, X10))
...
sigma <- 2.3
 ...
 Y <- exp(b0 + b1*X1 + b3*X1^3 + b5*X1^5 + X3*b2*X1^2 + X3*b4*X1^4 + X3*b6 + errors)
 ...
 mylm <- lm(log(Y) ~ X1 + I(X3) + I(X1^2):X3 + I(X1^3) + I(X1^4):X3 + I(X1^5))
 summary(mylm)  
 #all p-values must be significant, except "(Intercept)"
 
#pairs(cbind(Y, X1, X2, X3, X4, X5, X7, X8, X9, X10))
