...
```{r}
plot(mylm, which = 1, main = "Residuals vs Fitted")
qqPlot(mylm$residuals, main = "QQ-Plot", ylab = "Residuals")
```
...
$$
  \underbrace{Y_i}_\text{annual sales} = \beta_0 + \beta_1 \underbrace{x_i}_\text{annual hours per worker} + \epsilon_i
$$
...
```{r}
mylm <- lm(tsales ~ hourspw, data = Clothing)
...
```{r}
# coninf for intercept
1745 + c(-1,1) * qt(1-0.05/2, 398) * 67479
# for slope
43885 + c(-1,1) * qt(1-0.05/2, 398) * 3320
```
...
new_data <- Clothing[-c(397),]
mylm <- lm(tsales ~ hourspw, data = new_data)
...
options(scipen = 5)
plot(log(tsales) ~ log(hourspw), data = new_data)
```
