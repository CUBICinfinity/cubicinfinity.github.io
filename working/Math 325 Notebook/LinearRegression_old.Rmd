...
# In the real world, we begin with data (like fabData) and try to recover the model that (we assume) was used to created it.
  ...
  fabLm <- lm(y ~ x, data = fabData) #Fit an estimated regression model to the fabData.

  summary(fabLm) #Summarize your model. 

  ggplot(data = fabData, aes(x=x, y=y)) + #Plot the data.
    geom_point() +
    geom_smooth(aes(color = "Estimated Line", linetype="Estimated Line"), method = "lm", se = FALSE) +
    geom_abline(aes(color = "True Line", linetype="True Line", intercept = beta0, slope = beta1)) +   
    scale_color_manual(name="", values = c("gray","blue")) +
    scale_linetype_manual(name="", values=c("solid", "dashed")) +
    theme_minimal() +
    theme(legend.position="top")
    
    #legend("topleft", legend = c("True Line", "Estimated Line"), lty=c(2,1), col=c("gray", "black"))
```
...
```{r, eval = TRUE}
  ...
  sigma <- 1.5 # the central limit theorem seems to hold for a sample size of 30, but if the sigma is large compared to the range of values, 30 will fail.
...
# In the real world, we begin with data (like fabData) and try to recover the model that (we assume) was used to created it.
  ...
  ggplot(data = fabData, aes(x=x, y=y)) + #Plot the data.
    geom_point() +
    geom_abline(slope = coef(fabLm)[2], intercept = coef(fabLm)[1]) + 
    geom_abline(intercept = beta0, slope = beta1, color = "gray", linetype="dashed") +
...
This equation makes several claims, or assumptions, about the error terms $\epsilon_i$, specifically 2, 3, and 5 of the regression assumptions are hidden inside the statement
$$
\epsilon_i \underbrace{\sim}_\text{Independent Errors} \overbrace{N}^\text{Normally distributed}(0, \underbrace{\sigma^2}_\text{Constant Variance})
$$
...
There are three important sums of square in regression.
...
text(4,1, "Dots Close to Line", cex=.8, col="gray")
...
text(3,5.4, paste("SSR =", round(sum((lmr$fit - mean(y))^2),2)), col="lightslategray", pos=1, cex=.8)
text(3,5, paste("SSTO =", round(sum((y - mean(y))^2),2)), col="lightslategray", pos=1, cex=.8)
text(4,1, "Dots Far from Line", cex=.8, col="gray")
...
text(3,5.4, paste("SSR =", round(sum((lmr$fit - mean(y))^2),2)), col="lightslategray", pos=1, cex=.8)
text(3,5, paste("SSTO =", round(sum((y - mean(y))^2),2)), col="lightslategray", pos=1, cex=.8)
text(4,1, "Dots Somewhat Away from Line", cex=.8, col="gray")
```
...
##### Inference for the Model Parameters{#infModelParam1}
