...
## Interactions
```{r, echo=F, warning=F}
data$Cause_of_death <- plyr::mapvalues(data$Cause_of_death, c("Strangled", "Blunt trauma"), c("Strangling", "Blunt force trauma"))

data$Gender <- plyr::mapvalues(data$Gender, "male", "Male")

close <- subset(data, Latitude < 30 & Latitude > 27)

data$Cause_of_death <- plyr::mapvalues(data$Cause_of_death, c("Strangled", "Blunt trauma"), c("Strangling", "Blunt force trauma"))

barplot(relcrit, beside=TRUE, ylim=c(0,53), xlab="How much they said they like being critiqued", legend=TRUE, args.legend = list(x="topleft", bty='n'))
```
