...
```
<font size="1"><br />
To save space, "Musical", "War", and "Western" were excluded from this plot as they each had less than 5 occurances; but they closely resemble the other genres with respect to IMDb score.
<br />
</font>
<br />
...
$$
We'll use a one-way ANOVA to see if it is statistically significant (Choosing 0.95 for the confidence level). "Sci-Fi" is not normally distributed, so I removed it from the analysis:
```{r, warning=FALSE}
movies2.aov <- aov(score ~ genre, data=subset(movies2, genre!="Sci-Fi"))
...
```
<br />"Sci-Fi" is not normally distributed and I removed it from the ANOVA. "Family" is close to being normally distributed so I chose to keep it.

This is the result of the one-way ANOVA without removing the "Sci-Fi" genre:
```{r, warning=FALSE}
