...
## Explanation
The OrchardSprays dataset contains the results of a test conducted in 1947. The purpose of the test was to see how well lime sulfur could prevent bees from pollinating trees.
Honey comb cells were filled with a sucrose solution. Bees were given two hours to collect the solution. In 64 samples, 8 different levels of lime sulfer were added to the solutions, 8 observations per level. The amount of the solution collected by the bees was recorded.
...
```{r}
hist(permutedTestStats, col = "orchid", xlim = c(0,20), breaks=50, main="Histogram of Permuted Test Statistics", xlab="Test Statistic")
...
Most levels are dramatically different from each other. A concentration of 1/2,500 (.0004) or more works well at repelling bees, depending on how much the bees need to be repelled.
